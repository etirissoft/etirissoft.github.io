...
$(window).load(function(){
	...
	if ( $('#protfolio-msnry1').length > 0 ) {
		// init Isotope
		var loading = 0;
		var portfolioMsnry = $('#protfolio-msnry1').isotope({
			itemSelector: '.single-port-item',
			layoutMode: 'fitRows'
		});


		$('#portfolio-msnry-sort1 a').on( 'click', function(e) {
		...
		$('#portfolio-item-loader').on( 'click', function(e) {
			...
			for (var i = 0; i < 3; i++) {
				$.get("portfolioitems.html", function(data, status){
					var lists, numb, target = $('#portfolio-msnry-sort1 li.active a').data('target');
		...
		$('#protfolio-msnry1').delegate('a.modal-trigger', 'click', function(e){
			e.preventDefault();
			var $this = $(this);
			portfolioModal.openModal({
				dismissible: true,
